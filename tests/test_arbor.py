...
def test_arbor():
    ...
    edges = skel.edges
    radii = skel.r
    ntype = skel.ntype if skel.ntype is not None else np.zeros(len(nodes), dtype=int)

    # +1 to match MATLAB indexing (1-based)
    nodes += 1
    if ntype[0] != 1:
        ntype[0] = 1  # set the soma type to 1, as in the original MATLAB code
    ...
    # warped_nodes = warped_arbor["skel"].nodes
